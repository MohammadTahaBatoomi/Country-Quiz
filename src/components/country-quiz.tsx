...
const CountryQuiz: React.FC<CountryQuizProps> = () => {
  const [current, setCurrent] = useState(1);
  const [selected, setSelected] = useState<number | null>(null);
  const [showResult, setShowResult] = useState(false);

  const q = questions[0]; // demo: only one question
    ...
    <div className="min-h-screen flex items-center justify-center mx-4">
      ...
      <div className="bg-[#343963] rounded-2xl shadow-2xl md:px-12 px-6 py-10 w-full max-w-2xl flex flex-col gap-8">
        ...
        <div className="flex justify-center md:gap-2 gap-1 mb-2">
          {[...Array(10)].map((_, i) => (
            <button
              key={i}
              className={
                `md:w-12 md:h-12 w-9 h-8 rounded-full flex items-center justify-center text-lg font-bold transition-all duration-5  00 ` +
                (i === current - 1
        ...
        <div className="text-center">
          <h2 className="text-xl font-bold px-0 md:px-23">
          ...
          {q.options.map((opt, idx) => {
            ...
            if (selected !== null) {
              if (idx === selected && selected !== q.answer) {
                ...
                icon = <img src="images/resources/Close_round_fill.svg" className="pl-2" />;
              } else if (idx === q.answer) {
                btnClass +=
                  "bg-[#393f6f] text-blue-200";
                icon = <img src="images/resources/Check_round_fill.svg" className="pl-2" />;
              } else {
                btnClass +=
